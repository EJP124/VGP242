...
void GraphicsSystem::Initialize(HWND window, bool fullscreen)
{
	...
	ASSERT(SUCCEEDED(hr), "GraphicsSystem: failed to initailize device or swap chain");
...
void GraphicsSystem::Terminate()
{
	...
	SafeRelease(mImmdiateContext);
	SafeRelease(mD3DDevice);
}

void GraphicsSystem::BeginRender()
{
	mImmdiateContext->OMSetRenderTargets(1, &mRenderTargetView, mDepthStencilView);
	mImmdiateContext->ClearRenderTargetView(mRenderTargetView, (FLOAT*)(&mClearColor));
	mImmdiateContext->ClearDepthStencilView(mDepthStencilView, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0.0f);
}
...
{
	mImmdiateContext->OMSetRenderTargets(0, nullptr, nullptr);
	...
	if (width != GetBackBufferWidth() || height != GetBackBufferHeight())
	{
		...
		ASSERT(SUCCEEDED(hr), "GraphicsSystem: failed to assess swap chain view");
	...
	ASSERT(SUCCEEDED(hr), "GraphicsSystem: failed to create depth stencil");
	...
	mImmdiateContext->RSSetViewports(1, &mViewPort);
}
...
float KTEngine::Graphics::GraphicsSystem::GetBackBufferAspectRatio() const
{
